...
function StandupForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">What did you do today? *</label>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Blockers</label>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Plan for tomorrow</label>
        <textarea rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
  ...
  return (
    <div className="space-y-6">
                ...
                <CardBody>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-slate-900 dark:text-white text-sm">{update.profiles.full_name}</span>
                    </div>
                    {update.blockers && <Badge variant="red"><AlertTriangle size={10} className="mr-1" />Blocker</Badge>}
                  </div>
                  <div className="mt-3 space-y-2 text-sm">
                    <div>
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Did Today</span>
                      <p className="text-slate-700 dark:text-slate-200 mt-0.5">{update.did_today}</p>
                    </div>
                    {update.blockers && (
                      <div>
                    ...
                    {update.plan_tomorrow && (
                      <div>
                        <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Tomorrow</span>
                        <p className="text-slate-700 dark:text-slate-200 mt-0.5">{update.plan_tomorrow}</p>
                      </div>
                    )}
                  </div>
                </CardBody>
      ...
      {!Object.keys(grouped).length && (
        <p className="text-center text-slate-400 dark:text-slate-500 py-12 text-sm">No standups in the last 7 days</p>
      )}
